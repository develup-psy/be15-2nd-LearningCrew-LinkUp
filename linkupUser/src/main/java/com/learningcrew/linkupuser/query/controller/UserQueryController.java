...
public class UserQueryController {
    ...
    public UserInfoResponse getUser(@PathVariable int userId) {
        return userQueryService.getUserInfo(userId);
    }

    @GetMapping("/me/meetings/{memberId}")
    public ResponseEntity<ApiResponse<MeetingMemberDto>> getMeetingUser(@PathVariable("memberId") int memberId){
        log.info("모임 조회 memberId: {}", memberId);
        return ResponseEntity.ok(ApiResponse.success(userQueryService.getMeetingMember(memberId)));
    }

    @GetMapping("/me/{userId}/exists")
    public Boolean existsUser(@PathVariable("userId") int userId){
